...
(
	...
	SELECT 1 FROM `assignments` WHERE
		`assignments`.id<>'.$_POST['assignment'].' AND
...
{
	message ( 'Updated deadline can\'t be earlier than tomorrow', 2 );
...
(
	...
	UPDATE `assignments` SET
		`assignments`.course_id='.$_POST['course'].',
		`assignments`.title='.sql_escape ( $_POST['title'], 50 ).',
		`assignments`.deadline=\''.date ( 'Y-m-d H:i:s', strtotime ( $_POST['deadline'] ) ).'\'
	WHERE `assignments`.id='.$_POST['assignment'], 1 )
)
